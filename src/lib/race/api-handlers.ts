...
export async function handleRegisterLap(
  ...
  try {
    const result = await db.registerLap(supabase, bib, editionYear);
...
export async function handleAddRunner(
  ...
  try {
    const existing = await db.getRunners(supabase, editionYear);
    ...
    const runner = await db.addRunner(supabase, {
      bib: finalBib,
      name: name.trim(),
...
export async function handleDeleteLap(
  ...
  try {
    await db.deleteLap(supabase, lapId);
    return { ok: true };
  } catch (err: any) {
    return { ok: false, error: err.message };
  }
